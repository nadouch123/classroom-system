document.addEventListener('DOMContentLoaded', () => {
    ...
    const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."; // Paste full key
    ...
    // NEW: Schedule Builder Data Structure
    let scheduleData = {
        classroom: "111",
    ...
    async function fetchDevices() {
        ...
        if(isConnected) {
            ...
            showStatus("Requesting Device List...", "blue");
        } else {
    ...
    addSlotBtn.addEventListener('click', () => {
        ...
        if (!day || !time) return alert("Please select Day and Time");
        if (!subject) return alert("Please enter Subject");

        // Add to local data structure
        scheduleData.schedule[day].push({
            start: time.split(':')[0], // Start HH:MM
            end: time.split(':')[1],   // End HH:MM
    ...
    resetScheduleBtn.addEventListener('click', () => {
        ...
        days.forEach(day => {
            scheduleData.schedule[day] = []; // Clear all days
        });
    ...
    // 3. Render Preview (Updates Day List)
    function renderPreview() {
        let html = '<div class="space-y-2">';
        
        // Loop through ALL 7 days (Mon-Sun)
        ...
        allDays.forEach(day => {
            ...
            // List
            if (count > 0) {
                html += `<ul class="list-disc pl-4 text-sm space-y-1">`;
                slots.forEach(slot => {
                    html += `
                        <li class="flex justify-between items-center bg-white p-1 rounded shadow-sm border border-gray-100">
                            <div class="font-bold text-gray-800">${slot.start} - ${slot.end}: ${slot.subject}</div>
                            <span class="text-xs text-gray-500">${slot.professor} | ${slot.section}</span>
                        </li>
                    `;
                });
                html += `</ul>`;
            } else {
                html += `<p class="text-gray-400 text-xs italic">No slots</p>`;
            }
    ...
    // 4. Helper: Check if Ready to Send
    function updateSendButton() {
        // Count total slots across ALL 7 days
        let total = 0;
        const allDays = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"];
    ...
    sendScheduleBtn.addEventListener('click', async () => {
        ...
        // FIX: Calculate total locally to avoid undefined variable error
        let total = 0;
        ...
        try {
            ...
            for (const opt of selectedOptions) {
                ...
                try {
                    ...
                    console.error(e);
                    showStatus(`Error sending to ${opt.value}`, "error");
                } 
            } 
...
}); // <--- THIS WAS MISSING. It closes document.addEventListener