document.addEventListener('DOMContentLoaded', () => {
    ...
    const slotStartTime = document.getElementById('slotStartTime'); // NEW
    const slotEndTime = document.getElementById('slotEndTime');   // NEW
    ...
    const slotError = document.getElementById('slotError'); // NEW Error display
    ...
    function isOverlapping(startA, endA, startB, endB) {
        // Overlap logic: If StartA is before EndB AND EndA is after StartB
        return (startA < endB) && (endA > startB);
    }

    // ====== BUILDER LOGIC ======
    
    // 1. Add Slot (Updated with Conflict Detection)
    addSlotBtn.addEventListener('click', () => {
        ...
        // Add to local data structure (Storing full HH:MM strings)
        ...
        // Sort slots by start time for better viewing
    ...
    function renderPreview() {
        ...
        allDays.forEach(day => {
            ...
            if (count > 0) {
                ...
                slots.forEach(slot => {
                    html += `
                        <div class="flex justify-between items-center bg-white p-2 rounded shadow-sm border border-gray-100 text-left">
                            <div class="font-bold text-gray-800 text-sm">
                                <span class="text-blue-600">${slot.start}</span> - <span class="text-red-500">${slot.end}</span> : ${slot.subject}
                            </div>
                            <span class="text-xs text-gray-500 ml-2">${slot.professor} | ${slot.section}</span>
                        </div>
                    `;
                });
                html += `</div>`;
